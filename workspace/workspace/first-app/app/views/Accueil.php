<!DOCTYPE html>
<html lang="fr">

<head>
    <meta charset="UTF-8">
    <meta name="description" content="">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">

    <!-- Title -->
    <title>G-Mobile - Mobiles App HTML Template</title>


    <!-- Stylesheet -->
    <link rel="stylesheet" href="/workspace/first-app/app/styles/style.css">


</head>
<Body>
<?php
 require_once ('C:\xampp\htdocs\workspace\first-app\app\views\MenuGestionnaire.php');
       
?>

<!-- Welcome Area Start -->
<section class="welcome-area">
        <div class="welcome-slides owl-carousel">
            <!-- Single Welcome Slide -->
            <div class="single-welcome-slide bg-img" style="background-image: url(/workspace/first-app/app/img/bg-img/48.jpg);">
                <!-- Welcome Content -->
                <div class="welcome-content h-100">
                    <div class="container h-100">
                        <div class="row h-100 align-items-center">
                            <!-- Welcome Text -->
                            <div class="col-12 col-md-9 col-lg-6">
                                <div class="welcome-text">
                                    <h2 data-animation="fadeInUp" data-delay="100ms">G-Mobile.. <br> Nous gérons vos mobiles.</h2>
                                    <p data-animation="fadeInUp" data-delay="400ms">“G-Mobile vous accompagne.”</p>
                                    
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>

            <!-- Single Welcome Slide -->
           <div class="single-welcome-slide bg-img" style="background-image: url(/workspace/first-app/app/img/bg-img/54.jpg);">
                <!-- Welcome Content -->
                <div class="welcome-content h-100">
                    <div class="container h-100">
                        <div class="row h-100 align-items-center">
                            <!-- Welcome Text -->
                            <div class="col-12 col-md-9 col-lg-6">
                                <div class="welcome-text">
                                    <h2 data-animation="fadeInUp" data-delay="100ms">G-Mobile.. <br> Nous gérons vos mobiles.</h2>
                                    <p data-animation="fadeInUp" data-delay="400ms">“...”</p>
                                   
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- Welcome Area End -->
    </header>
    <!-- Header Area End -->
    
    <!-- About Area Start -->
    <section class="akame-about-area section-padding-80-0">
        <div class="container">
            <div class="row align-items-center">
                <!-- Section Heading -->
                <div class="col-12 col-md-6 col-lg-4">
                    <div class="section-heading mb-80">
                        <h2>G-Mobile</h2>
                        <p>L'Application n° 1 en France de gestion des mobiles professionnels</p>
                        <span>About Us</span>
                    </div>
                </div>

                <!-- About Us Thumbnail -->
                <div class="col-12 col-md-6 col-lg-4">
                    <div class="about-us-thumbnail mb-80">
                        <img src="/workspace/first-app/app/img/bg-img/56.jpg" alt="">
                    </div>
                    <div class="about-us-thumbnail mb-80">
                        <img src="/workspace/first-app/app/img/bg-img/51.jpg" alt="">
                    </div>
                </div>

                <!-- About Us Content -->
                <div class="col-12 col-lg-4">
                    <div class="about-us-content mb-80 pl-4">
                        <h3>Dans le cadre des métiers de votre entreprise</h3>
                        <p>“Optimiser et suivre les performances de vos travailleurs itinérants”</p>
                        <a href="#" class="btn akame-btn active mt-30">Read More</a>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- About Area End -->

    <!-- Border -->
    <div class="container">
        <div class="border-top mt-3"></div>
    </div>

    <!-- Our Service Area Start -->
    <section class="akame-service-area section-padding-80-0">
        <div class="container">
            <div class="row">
                <div class="col-12">
                    <!-- Section Heading -->
                    <div class="section-heading text-center">
                        <h2>Nos Services</h2>
                        <p>G-Mobile propose des solutions, simples, efficaces, immédiatement opérationnelles. </p>
                    </div>
                </div>
            </div>

            <div class="row">

                <!-- Single Service Area -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-service-area mb-80 wow fadeInUp" data-wow-delay="200ms">
                        <img src="/workspace/first-app/app/img/core-img/s1.png" alt="">
                        <h5>Force & choix de vente</h5>
                    </div>
                </div>

                <!-- Single Service Area -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-service-area mb-80 wow fadeInUp" data-wow-delay="400ms">
                        <img src="/workspace/first-app/app/img/core-img/s3.png" alt="">
                        <h5>Préparation de commande</h5>
                        
                    </div>
                </div>

                <!-- Single Service Area -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-service-area mb-80 wow fadeInUp" data-wow-delay="600ms">
                        <img src="/workspace/first-app/app/img/core-img/s4.png" alt="">
                        <h5>Maintenance assurée</h5>
                        
                    </div>
                </div>

                <!-- Single Service Area -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-service-area mb-80 wow fadeInUp" data-wow-delay="800ms">
                        <img src="/workspace/first-app/app/img/core-img/s3.png" alt="">
                        <h5>Transport/ Livraison</h5>
                       
                    </div>
                </div>

            </div>
        </div>
    </section>
    <!-- Our Service Area End -->

   

    

    <!-- Footer Area Start -->
     <!-- Footer Area Start -->
     <footer class="footer-area section-padding-80-0">
           <!-- Contact Information Area Start -->
    <section class="contact-information-area section-padding-80-0">
        <div class="container">
            <div class="row">
                <!-- Single Contact Information -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-contact-information mb-80">
                        <i class="icon_phone"></i>
                        <h4>Téléphone</h4>
                        <p>+33-145215485</p>
                    </div>
                </div>

                <!-- Single Contact Information -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-contact-information mb-80">
                        <i class="icon_pin"></i>
                        <h4>Addresse</h4>
                        <p>23 Boulevard de France, Paris, 75008</p>
                    </div>
                </div>

                <!-- Single Contact Information -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-contact-information mb-80">
                        <i class="icon_clock"></i>
                        <h4>Ouvert de </h4>
                        <p>9:00 à 19:00 pm</p>
                    </div>
                </div>

                <!-- Single Contact Information -->
                <div class="col-12 col-sm-6 col-lg-3">
                    <div class="single-contact-information mb-80">
                        <i class="icon_mail"></i>
                        <h4>Email</h4>
                        <p>info.g-mobile@gmail.com</p>
                    </div>
                </div>
            </div>
        </div>
    </section>
    <!-- Contact Information Area End -->
 
    

    <!-- All JS Files -->
    <!-- jQuery -->
    <script src="/workspace/first-app/app/js/jquery.min.js"></script>
    <!-- Popper -->
    <script src="/workspace/first-app/app/js/popper.min.js"></script>
    <!-- Bootstrap -->
    <script src="/workspace/first-app/app/js/bootstrap.min.js"></script>
    <!-- All Plugins -->
    <script src="/workspace/first-app/app/js/akame.bundle.js"></script>
    <!-- Active -->
    <script src="/workspace/first-app/app/js/default-assets/active.js"></script>

</body>

</html>